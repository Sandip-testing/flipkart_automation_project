...
OPTION_BASE_URL = "https://www.nseindia.com/get-quotes/derivatives?symbol=NIFTY&identifier=OPTIDXNIFTY"
...
def extract_spot_value(page: Page):
    ...
    try:
        ...
        if match:
            ...
            return original, rounded + 200, rounded - 200
...
def main():
    with sync_playwright() as p:
        ...
        try:
            global spot, plus_200, minus_200
            spot, plus_200, minus_200 = extract_spot_value(page)
            if not all([spot, plus_200, minus_200]):
                print("Failed to fetch spot values.")
                return

            print(f"Spot: {spot}, spot + 200: {plus_200}, spot - 200: {minus_200}")

            # Process PUT (Spot +200)
            process_option(
                page, "PE", plus_200,
                SELECTORS["PE_IV"], SELECTORS["PE_VWAP"], is_put=True
            )

            # Process CALL (Spot -200)
            process_option(
                page, "CE", minus_200,
